<!-- see model/extension/postfinancecheckout/order.php & PostFinanceCheckoutAdministration.ocmod.xml -->
<?php foreach($job_groups as $key => $job_group) : ?>
	<a class='h3' data-toggle='collapse' href='#postfinancecheckout-jobs-<?php echo $key;?>' aria-controls='postfinancecheckout-jobs-<?php echo $key;?>' aria-expanded='<?php echo $key == 0 ? 'true' : 'false'?>'
	><?php echo $job_group['title']; ?> <i class='fa fa-chevron-down'></i></a>
	
	<div id='postfinancecheckout-jobs-<?php echo $key; ?>' class='collapse'>
